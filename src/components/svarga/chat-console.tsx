  ...
  return (
    <div className="relative rounded-3xl bg-ink p-6 shadow-2xl shadow-ink/20">
      ...
      <Conversation className="h-[340px]"><ConversationContent className="gap-4 p-0">
        ...
        {status === "submitted" || rendering ? <Shimmer className="font-mono text-[11px] uppercase tracking-widest">{rendering ? "Painting your vision…" : "Reasoning across traditions…"}</Shimmer> : null}
      ...
      <PromptInput className="mt-4 rounded-2xl border-cream/10 bg-cream/5" onSubmit={(_message, event) => { event.preventDefault(); send(input); }}>
        <PromptInputTextarea value={input} onChange={(event) => setInput(event.target.value)} placeholder={mode === "image" ? "Describe the image Svarga should create…" : "Ask Svarga anything…"} className="min-h-16 text-cream placeholder:text-cream/40" />
        <PromptInputFooter className="justify-end border-cream/10"><PromptInputSubmit status={rendering ? "submitted" : status} onStop={stop} disabled={busy || input.trim().length === 0} className="bg-crimson text-cream hover:bg-crimson/90" /></PromptInputFooter>
      </PromptInput>
    </div>
  );
}
